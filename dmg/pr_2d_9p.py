    ...
    for k in range(l):
        for i in range(m_1d):
            for j in range(m_1d):
                for si in range(3):
                    for sj in range(3):
                        Y[2 * (n_1d * i + j) + n_1d * si + sj, k] += P[si, sj] * X[m_1d * i + j, k]
    return Y
    ...
    for k in range(l):
        for i in range(m_1d):
            for j in range(m_1d):
                for si in range(3):
                    for sj in range(3):
                        Y[si, sj] += g[2 * (n_1d * i + j) + n_1d * si + sj, k] * X[m_1d * i + j, k]
    return Y
    ...
    for k in range(l):
        for i in range(m_1d):
            for j in range(m_1d):
                for si in range(3):
                    for sj in range(3):
                        Y[m_1d * i + j, k] += P[si, sj] * g[2 * (n_1d * i + j) + n_1d * si + sj, k]
    return Y
    ...
    for k in range(l):
        for i in range(m):
            for j in range(m):
                for si in range(3):
                    for sj in range(3):
                        Y[m * i + j, k] += R[si, sj] * X[2 * (n_1d * i + j) + n_1d * si + sj, k]
    return Y
    ...
    for k in range(l):
        for i in range(m):
            for j in range(m):
                for si in range(3):
                    for sj in range(3):
                        Y[si, sj] += g[m * i + j, k] * X[2 * (n_1d * i + j) + n_1d * si + sj, k]
    return Y
    ...
    for k in range(l):
        for i in range(m):
            for j in range(m):
                for si in range(3):
                    for sj in range(3):
                        Y[2 * (n_1d * i + j) + n_1d * si + sj, k] += R[si, sj] * g[m * i + j, k]
    return Y
                        ...
                        for s1j in range(3):
                            ...
                            for s2i in range(smin_i, smax_i):
                                ...
                                for s2j in range(smin_j, smax_j):
                                    ...
                                    Anew[i, j, si, sj] += R[s1i, s1j] * P[s2i, s2j] * A[2*i+s1i, 2*j + s1j, sloci, slocj]
    return Anew

@jit(nopython=True)
def generate_RAP_R_vjp_inner(g, R, A, P):
    m = g.shape[0]
    res = pure_np.zeros((3, 3))
    for i in range(m): ##P'AR is strange... P_{ki} A_{kl} R_{lj} -> it is also 3-diagonal
        # 0 <= i + s - 1 < m
        # s <= m + 1
        for j in range(m):
                    ...
                    for s1i in range(3): #
                        ...
                        for s1j in range(3):
                            ...
                            for s2i in range(smin_i, smax_i):
                                ...
                                for s2j in range(smin_j, smax_j):
                                    slocj = 2 * jj + s2j - 2 * j - s1j + 1
                                    res[s1i, s1j] += g[i, j, si, sj] * P[s2i, s2j] * A[2*i+s1i, 2*j + s1j, sloci, slocj]
    return res
                    ...
                    for s1i in range(3): #
                        ...
                        for s1j in range(3):
                            ...
                            for s2i in range(smin_i, smax_i):
                                ...
                                for s2j in range(smin_j, smax_j):
                                    slocj = 2 * jj + s2j - 2 * j - s1j + 1
                                    res[2*i+s1i, 2*j + s1j, sloci, slocj] += R[s1i, s1j] * P[s2i, s2j] * g[i, j, si, sj]
    return res
                    ...
                    for s1i in range(3): #
                        ...
                        for s1j in range(3):
                            ...
                            for s2i in range(smin_i, smax_i):
                                ...
                                for s2j in range(smin_j, smax_j):
                                    slocj = 2 * jj + s2j - 2 * j - s1j + 1
                                    res[s2i, s2j] += R[s1i, s1j] * g[i, j, si, sj] * A[2*i+s1i, 2*j + s1j, sloci, slocj]
    return res
...
def generate_from_P_R_lists(P_list, R_list, fine_dims):
    ...
    for i, (P, R) in enumerate(zip(P_list, R_list)):
        current_pr = PR_2d_9p(fine_dims[i], R, P)
        PR_list.append(current_pr)
    return PR_list