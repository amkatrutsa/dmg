...
@jit(nopython=True)
def P_matvec_batch_inner(g, P, X):
    m, l = X.shape
...
def P_matvec_batch_P_vjp(g, ans, P, X, fine_dim):
    return P_matvec_batch_inner(g, P, X)
        ...
        for s in range(sb_min, sb_max):
            ...
            for s1 in range(3):
                ...
                for s2 in range(smin, smax):
                    ...
                    Anew[i, s] = Anew[i, s] + R[s1] * P[s2] * A[2*i+s1, sloc]
    ...
    for i in range(m): 
        ...
        for s in range(sb_min, sb_max):
            ...
            for s1 in range(3): #
                ...
                for s2 in range(smin, smax):
                    sloc = 2 * j + s2 - 2 * i - s1 + 1
                    res[s1] = res[s1] + g[i, s]*P[s2]*A[2*i+s1, sloc]
    ...
    for i in range(m): 
        ...
        for s in range(sb_min, sb_max):
            ...
            for s1 in range(3): #
                ...
                for s2 in range(smin, smax):
                    sloc = 2 * j + s2 - 2 * i - s1 + 1
                    res[2*i+s1, sloc] = res[2*i+s1, sloc] + R[s1] * P[s2] * g[i, s]
    return res
    ...
    for i in range(m): 
        ...
        for s in range(sb_min, sb_max):
            ...
            for s1 in range(3): #
                ...
                for s2 in range(smin, smax):
                    sloc = 2 * j + s2 - 2 * i - s1 + 1
                    res[s2] = res[s2] + R[s1]*g[i, s]*A[2*i+s1, sloc]
                    #Anew[i, s] = Anew[i, s] + R[i, s1] * P[j, s2] * A[2*i+s1, sloc]       
    return res
...
class PR_1d_3p(pr_base.PR):
    def __init__(self, n, P=None, R=None):
        m = (n + 1) // 2 - 1
        if m % 2 == 0:
            m -= 1
        if P is None and R is None:
            P = 0.5 * np.array([1.0, 2.0, 1.0])
            R = 0.25 * np.array([1.0, 2.0, 1.0])
...
def generate_from_P_R_lists(P_list, R_list, fine_dims):
    ...
    for i, (P, R) in enumerate(zip(P_list, R_list)):
        current_pr = PR_1d_3p(fine_dims[i], P, R)
        PR_list.append(current_pr)
    return PR_list